...
class employeeClass:
    def __init__(self,root):
       ...
       #row2========
        ...
        txt_pass=Entry(self.root,textvariable=self.var_pass,show="*",font=("goudy old style",15),bg="white").place(x=500,y=230,width=180)
            ...
            if self.var_searchby.get()=="Select":
                ...
                messagebox.showerror("Error","Select input should be required",parent=self.root)
                 ...
                 if self.var_searchby.get()=="Email":
                     ...
                     search.set("name")
                 cur.execute("select * from employee where "+str(search.get())+"= '"+str(self.var_searchtxt.get())+"'")
                 rows=cur.fetchall()
                 if len(rows)!=0:
                      self.EmployeeTable.delete(*self.EmployeeTable.get_children())  
                      for row in rows:
