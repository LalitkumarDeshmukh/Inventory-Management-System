...
#################TITLE########## 
    ...
    #product details======
        ...
        self.product_table=ttk.Treeview(p_frame,columns=("pid","Supplier","Category","name","qty","price","status"),yscrollcommand=scrolly.set,xscrollcommand=scrollx.set)
        ...
        self.product_table.heading("Supplier",text="Supplier name")
    ...
    def add(self):
        ...
        try:
            ...
            if self.var_cat.get()=="Select" or self.var_cat.get()=="Empty" or self.var_sup.get()=="Select" or self.var_name.get()=="": 
                ...
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Product already available, try different",parent=self.root)
                else:
                    cur.execute("Insert into product (Category,Supplier,name,qty,price,status) values(?,?,?,?,?,?)",(
                                              self.var_cat.get(),
                                              self.var_sup.get(),
                                              self.var_name.get(),
                                              self.var_qty.get(),
    ...
    def update(self):
        ...
        try:
            if self.var_pid.get()=="": 
                ...
                cur.execute("Select * from product where pid=?",(self.var_emp_id.get(),))
                row=cur.fetchone()
                if row==None:
        ...
        try:
            if self.var_searchby.get()=="Select":
                ...
                messagebox.showerror("Error","Select input should be required",parent=self.root)
                 ...
                 searchby=StringVar()
                 if self.var_searchby.get()=="Name":
                     searchby.set("name")
                 elif self.var_searchby.get()=="Category":
                         searchby.set("Category")
                 else:
                         searchby.set("Supplier")
                 cur.execute("select * from product where "+str(searchby.get())+"= '"+str(self.var_searchtxt.get())+"'")
                 rows=cur.fetchall()
                 if len(rows)!=0:
                      self.product_table.delete(*self.product_table.get_children())  
                      for row in rows:
